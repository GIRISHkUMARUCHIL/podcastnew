...
// Disable TML update nag - V7+ Breaks functionality
function pro_studiosfilter_plugin_updates( $value ) {
  if( isset( $value->response['theme-my-login/theme-my-login.php'] ) ) {        
     unset( $value->response['theme-my-login/theme-my-login.php'] );
   }
   return $value;
}
add_filter( 'site_transient_update_plugins', 'pro_studiosfilter_plugin_updates' );
