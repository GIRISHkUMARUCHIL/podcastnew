 ...
 */
$source = $text = $link = $google_fonts = $font_container = $el_id = $el_class = $css = $css_animation = $font_container_data = $enable_fittext = $fittex_size = $inline_block_display = $google_fonts_data = array();
...
if( 'yes' === $enable_fittext ) {
	$wrapper_attributes[] = 'data-fitText="true"';
	$wrapper_attributes[] = ! empty( $fittex_size ) ? 'data-max-fontSize="' . $fittex_size . '"' : '';
}

if( 'yes' === $inline_block_display ) {
	$css_class = $css_class.' sr_it-inline-block';
}

if( ! empty( $letter_spacing ) ) {
	$styles[] = 'letter-spacing:' . esc_attr( $letter_spacing ) . '';	
}
