...
?>			<script language="JavaScript">
				jQuery( document ).ready( function() {
					...
					jQuery(".file-upload-button").click( function( event ) {
						...
						custom_file_frame.on( "select", function() {
								...
								activeFileUploadContext.find('.file-upload-text').val(attachment.attributes.url).trigger('change');
...
<?php		if ( isset( $page['description'] ) && ! empty( $page['description'] ) ) { 
			...
			if ( isset( $page['subpages'] ) /*&& count( $page['subpages'] > 0 )*/) {
				...
				foreach ($page['subpages'] as $subpage) {
					echo '<a class="nav-tab" data-tab="#' . $subpage['menu_slug'] . '" href="' . get_admin_url( null , "admin.php?page=" . $page['menu_slug'] ) . '#' . $subpage['menu_slug'] . '">' . $subpage["page_title"] . '</a>';
				}

				echo '</h2>';
				echo '<div class="tab-container">';
				foreach ($page['subpages'] as $subpage) {
					echo '<div class="' . $subpage['menu_slug'] . ' tab-container-single">';
					$this->build_subpage( $subpage );
					echo '</div>';
				}
				echo '</div>';
			}
...
?>		</div>
	 ...
	 * @param array page Page properties
	 */
	private function build_subpage( $page ) {
		$this->options[ $page['menu_slug'] ] = get_option( '_' . $page['menu_slug'] . '_options' );
		...
		if ( isset( $page['sections'] ) && count( $page['sections'] ) > 0 ) {
			...
			settings_fields( $page['menu_slug'] . '_group' );	// option_group
			do_settings_sections( $page['menu_slug'] );			// page
			submit_button();
			echo'</form>';
		}
	...
	private function build_field( $field, $page ) {
		switch ( $field['type'] ) {
			...
			case 'htmlExport':
				...
				$data_export = array();
				echo '<textarea readOnly="true" class="export">';
				foreach ($field['export_options'] as $option) {
					$value = get_option( $option );
					if ( !empty( $value ) ) {
						array_push($data_export, array($option => $value) );
					}
			...
			case 'password':
				...
				$field_value = ( isset( $this->options[ $page ][ $field['id'] ] )  && $this->options[ $page ][ $field['id'] ] != ''  ) ? $this->options[ $page ][ $field['id'] ] : ( ( isset( $field['value'] ) ) ? $field['value'] : '' );
			...
			case 'typography':
				...
				$field_disabled = ( isset( $field['args']['disabled'] ) && boolval( $field['args']['disabled'] ) === true ) ? 'disabled' : '';
				$field_description = ( isset( $field['description'] ) && !empty( $field['description'] ) ) ? '<p class="description">' . $field['description'] . '</p>' : '';
				$sr_font_readable = ( isset($field_value['font_readable']) && !empty($field_value['font_readable']) ) ? $field_value['font_readable'] : '';
				printf('<label class="%s">
						<input type="text" class="if_font" name="%s[%s][font]" value="%s">
						<input type="hidden" class="if_font_font" name="%s[%s][font_readable]" value="%s">
						<input type="hidden" class="if_font_weight" name="%s[%s][weight]" value="%s">
						<input type="text" name="%s[%s][size]" value="%s">
						<input type="text" class="color" name="%s[%s][color]" value="%s">
						%s
					</label>',
					$field['type'],
					'_' . $page . '_options',
	...
	private function create_nav_tab( $navTabs ){
