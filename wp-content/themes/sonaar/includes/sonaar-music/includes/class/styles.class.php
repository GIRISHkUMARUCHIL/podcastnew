		...
		public function setFont($element, $option_key, $important = false) {
			...
			if($this->use_options) {
				...
				if(!empty($font) && is_string($font_family) && strpos($font, "_safe_") === false) {
					$fonts[md5($font_family)] = array(
						'font-family' => $font_family
					);
					if(isset($this->google_fonts[md5($font_family)])) {
						@$fonts[md5($font_family)]['font-weight'] = '' . $this->google_fonts[md5($font_family)]['font-weight'] . ',' . $this->get_option($option_key, 'weight') . '' ;
					}else{
						$fonts[md5($font_family)]['font-weight'] = $this->get_option($option_key, 'weight') ;
					}
				}
