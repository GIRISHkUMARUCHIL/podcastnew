<?php
	...
	function iron_sonaar_vc_setting_field_taxonomy_multiselect($settings, $value) {
		...
		$multiple = ( !empty($settings["single"] )? false : true );
		...
		$dropdown .= '<select name="'.$settings['param_name'].'_select" class="wpb_vc_param_value wpb-input wpb-select" '.(($multiple)? 'multiple="multiple':'') .' ">';
