...
/**
 * Class Sonaar_Vc_Templates_Panel_Editor
 * @since 1.0
 */
class Sonaar_Vc_Templates_Panel_Editor {
	/**
	 ...
	 * @var sonaarl
	 */
	protected $sonaar_templates = false;
	/**
	 * @since 4.4
	...
	public function init() {
		...
		// $enable_sonaar_universe = sonaar_helper()->get_option( 'enable-sonaar-universe' );
		
		$enable_sonaar_universe = Iron_sonaar::getOption('enable-sonaar-universe');
		if( ! $enable_sonaar_universe ) {
	...
	public function addTemplatesTab( $data ) {
		$newCategory = array(
			'category'        => 'sonaar_templates',
			'category_name'   => esc_html__( 'Sonaar Vault', 'sonaar' ),
			'category_weight' => 1,
	...
	protected function get_template_categories() {
		...
		$categories = array(
			'all'         => esc_html__( 'All', 'sonaar' ),
			'audioplayers' => esc_html__( 'Audio Players', 'sonaar' ),
			'artist'      => esc_html__( 'Artist', 'sonaar' ),
			'banners'	=> esc_html__( 'Banners', 'sonaar' ),
			'band_members'	=> esc_html__( 'Band Members', 'sonaar' ),
			'biography'   => esc_html__( 'Biography', 'sonaar' ),
			'contact'     => esc_html__( 'Contact & Booking', 'sonaar' ),
			'form'  	  => esc_html__( 'Form', 'sonaar' ),
			'footers'      => esc_html__( 'Footers', 'sonaar' ),
			'headings'      => esc_html__( 'Headings', 'sonaar' ),
			'misc'      => esc_html__( 'Misc', 'sonaar' ),
			'newsletter'      => esc_html__( 'Newsletter', 'sonaar' ),
			'podcast'      => esc_html__( 'Podcast', 'sonaar' ),
			'shop'        => esc_html__( 'Shop', 'sonaar' ),
			'team'        => esc_html__( 'Team', 'sonaar' ),
			'events_list'        => esc_html__( 'Tour dates', 'sonaar' ),
			'videos'     => esc_html__( 'Video Players', 'sonaar' )
		);
	...
	public function renderTemplateBlock( $category ) {
		...
		if ( 'sonaar_templates' === $category['category'] ) {

			$category['output'] = '<div class="vc_col-md-2 sonaar-sorting-container">';
			$category['output'] .= $this->get_template_categories();
			$category['output'] .= '</div>';


			$category['output'] .= '
			<div class="vc_column vc_col-sm-12 sonaar-templates-container">
	...
	function renderTemplateWindow( $template_name, $template_data ) {
		...
		if ( 'sonaar_templates' === $template_data['type'] ) {
			return $this->renderTemplateWindowSonaarTemplates( $template_name, $template_data );
		}
	 ...
	 * @return string
	 */
	public function renderTemplateWindowSonaarTemplates( $template_name, $template_data ) {
		ob_start();
		...
		$preview_template_title = esc_attr( 'Preview template', 'sonaar' );
		$add_template_title = esc_attr( 'Add template', 'sonaar' );
...
HTML;
	...
	public function renderBackendTemplate() {
		...
		if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
			die( 'Error: Vc_Sonaar_Templates::renderBackendTemplate:1' );
		}
	 ...
	 * @param $templates
	 *
	 * vc_filter: vc_load_sonaar_templates_limit_total - total items to show
	 *
	 * @return array
	 */
	...
	public function getAllTemplates() {
		...
		$data = array();
		$sonaar_templates = $this->getTemplates();
		$category_templates = array();
		foreach ( $sonaar_templates as $template_id => $template_data ) {
			$category_templates[] = array(
				'unique_id' => $template_id,
				'name' => $template_data['name'],
				'new' => isset( $template_data['new'] ) ? $template_data['new'] : false,
				'type' => 'sonaar_templates',
	 ...
	 * To modify you should use add_filter('vc_load_sonaar_templates','your_custom_function');
	 ...
	 *          ...
	 *      );
	 * Also see filters 'vc_load_sonaar_templates_panels' and 'vc_load_sonaar_templates_welcome_block' to modify
	 * templates in panels tab and/or in welcome block. vc_filter: vc_load_sonaar_templates - filter to override
	...
	public function loadDefaultTemplates() {
		...
		if ( ! is_array( $this->sonaar_templates ) ) {
			$this->sonaar_templates = $this->allTemplates();
		}

		return $this->sonaar_templates;
	}
	 ...
	 *
	 * @return array|sonaarl
	 */
	public function getDefaultTemplate( $template_index ) {

		$this->loadDefaultTemplates();
		if ( ! is_numeric( $template_index ) || ! is_array( $this->sonaar_templates ) || ! isset( $this->sonaar_templates[ $template_index ] ) ) {
			return false;
		}

		return $this->sonaar_templates[ $template_index ];
	}
	 ...
	 * @param $data
	 *
	 * @return sonaarl true if added, false if failed
	 */
	public function addDefaultTemplates( $data ) {
		if ( is_array( $data ) && ! empty( $data ) && isset( $data['name'], $data['content'] ) ) {
			if ( ! is_array( $this->sonaar_templates ) ) {
				$this->sonaar_templates = array();
			}
			$this->sonaar_templates[] = $data;
	 ...
	 * @since 4.4
	 *
	 * @param sonaarl $return | should function return data or not
	 *
	 * @return string
	 */
	...
	public function renderTemplateListItem( $template ) {
		$name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'sonaar' ) );
...
HTML;
		$output .= '<div class="sonaar-template-preview">';
		if ( $new ) { $output .= '<span class="sonaar-badge-new">New</span>'; }
		$output .= '<img src="'. esc_url( $template_image ) . '" data-src="' . esc_url( $template_image ) . '" alt="' . esc_attr( $name ) . '" width="300" height="200" /></div>';
		$output .= apply_filters( 'vc_templates_render_template', $name, $template );
		$output .= '<span class="sonaar-template-categories">' . esc_html( $template_sort_name ) . '</span>';
		$output .= <<<HTML
						</div>
