jQuery(function () {
  var savewarn = jQuery('#redux-opts-save-warn').clone();
  jQuery('#redux-opts-save-warn').remove();
  
  jQuery('.redux-typography-table .fontselect').fontselect();
  
  jQuery(".redux-typography-table .fontselect").on('change', function() {
    var this_id = jQuery(this).attr('id');
    var font = jQuery(this).val();
    font = font.replace('_safe_, ', '');
	font = font.replace('_safe_', '');
	font = font.replace(/\+/g, ' ').split(':');

	//Set italic
	if( typeof font[1] == 'string'){
		if(font[1].includes('italic') ){
			jQuery('#' + this_id + '-style').val('italic');
		}else{
			jQuery('#' + this_id + '-style').val('normal');
		}
	}

	//Set font-weight
	if( typeof font[1] == 'string'){
		if(font[1].includes('00') ){
			jQuery('#' + this_id + '-weight').val( font[1].charAt(0) + '00');
		}else{
			jQuery('#' + this_id + '-weight').val('400');
		}
	}
	
	//Set font-family
    jQuery('#' + this_id + '-example').css('font-family', font[0]);
    jQuery('#' + this_id + '-font_readable').val(font[0]);
    
  }).change();
    
  jQuery('.redux-typography-table .fs-size').on('click', function() {
	var the_original_id = jQuery(this).attr('id').split('-');
	var the_operation = the_original_id.pop();
	the_original_id_example = '#' + the_original_id.join('-') + '-example';
	the_original_id_size = '#' + the_original_id.join('-') + '-size';
	the_original_id_height = '#' + the_original_id.join('-') + '-height';

	var font_size = parseInt(jQuery(the_original_id_example).css('font-size').replace('px', ''));
	font_size = (the_operation == 'smaller') ? font_size-1 : font_size+1;
	jQuery(the_original_id_example).css('font-size',font_size + 'px');
	jQuery(the_original_id_size).val(font_size + 'px');
  });
  
  
  
  jQuery('.redux-typography-table .redux-typography-size').on('change', function() {
	  ...
	  var the_operation = the_original_id.pop();
	  the_original_id_example = '#' + the_original_id.join('-') + '-example';
	  jQuery(the_original_id_example).css('font-size',font_size);
  
  }).change();

  jQuery('.redux-typography-table .redux-typography-weight').on('change', function() {
	  ...
	  var the_operation = the_original_id.pop();
	  the_original_id_example = '#' + the_original_id.join('-') + '-example';
	  jQuery(the_original_id_example).css('font-weight',font_weight);
  
  });
  
  jQuery('.redux-typography-table .redux-typography-height').on('change', function() {
	  ...
	  var the_operation = the_original_id.pop();
	  the_original_id_example = '#' + the_original_id.join('-') + '-example';
	  jQuery(the_original_id_example).css('line-height',line_height);
  
  }).change();
  
  jQuery('.redux-typography-table .redux-typography-transform').on('change', function() {
  
  	  var transform = jQuery(this).val();
  	  ...
  	  if(transform != '') {
	  	...
	  	jQuery(the_original_id_example).css('text-transform',transform);
	  }	
  }).change();

  jQuery('.redux-typography-table .redux-typography-align').on('change', function() {
  	  ...
  	  if(text_align != '') {
	  	...
	  	var the_operation = the_original_id.pop();
	  	the_original_id_example = '#' + the_original_id.join('-') + '-example';
	  	jQuery(the_original_id_example).css('text-align',text_align);
	  }
 }).change();

 jQuery('.redux-typography-table .redux-typography-color').on('change', function() {
  	  
  	  var color = jQuery(this).val();
	  var the_original_id = jQuery(this).attr('id').split('-');
	  var the_operation = the_original_id.pop();
	  the_original_id_example = '#' + the_original_id.join('-') + '-example';
	  jQuery(the_original_id_example).css('color',color);
	  
 }).change();  
 
 jQuery('.redux-typography-table .redux-typography-bgcolor').on('change', function() {
  	  
  	  var bgcolor = jQuery(this).val();
	  var the_original_id = jQuery(this).attr('id').split('-');
	  var the_operation = the_original_id.pop();
	  the_original_id_example = '#' + the_original_id.join('-') + '-example';
	  jQuery(the_original_id_example).css('background-color',bgcolor);
	  
 }).change();   
 
 jQuery('.redux-typography-table .wp-picker-holder *').on('mousedown', function() {
  
  	  var color_input = jQuery(this);
  	  
  	  if(!jQuery(color_input).hasClass('wp-picker-holder'))
  	  	color_input = jQuery(this).closest('.wp-picker-holder');
  	  
  	  var color_input = jQuery(color_input).prev('.wp-picker-input-wrap').find('.wp-color-picker');
  	  var color = color_input.val();
	
	  var the_original_id = jQuery(color_input).attr('id').split('-');
	  var the_operation = the_original_id.pop();
	  the_original_id_example = '#' + the_original_id.join('-') + '-example';
	   	  
  	  if(color_input.hasClass('redux-typography-color')) {

	  	jQuery(the_original_id_example).css('color',color);
	  	
	  }else if(color_input.hasClass('redux-typography-bgcolor')) {
	  
		 jQuery(the_original_id_example).css('background-color',color); 
	  }	

  });

    
  jQuery('.redux-typography-table .redux-font-select.fs-reset').on('mousedown', function() {
	  
	  typography_table = jQuery(this).closest('.redux-typography-table');
	  the_original_example = typography_table.find('.redux-font-select.fs-example');
	  color_palette = typography_table.find('.wp-color-result');

	  typography_table.find('select,input[type=text]').val('');

	  jQuery(the_original_example).removeAttr('style');
	  jQuery(color_palette).removeAttr('style');

  });
  
  jQuery('.redux-typography-table .iris-square-handle.ui-slider-handle').on('click', function(e) {
	  e.preventDefault();
  });
  
  setTimeout(function() {
  	jQuery('#redux-opts-header').after(savewarn);
  },10);	
...
(function($){
  ...
  $.fn.fontselect = function(options) {
    ...
    var _fonts = [
		...
		"Fira+Sans+Condensed:100italic",
		"Fira+Sans+Condensed:200",
		"Fira+Sans+Condensed:200italic",
		"Fira+Sans+Condensed:300",
		"Fira+Sans+Condensed:300italic",
		"Fira+Sans+Condensed:regular",
		"Fira+Sans+Condensed:italic",
		"Fira+Sans+Condensed:500",
		"Fira+Sans+Condensed:500italic",
		"Fira+Sans+Condensed:600",
		"Fira+Sans+Condensed:600italic",
		"Fira+Sans+Condensed:700",
		"Fira+Sans+Condensed:700italic",
		"Fira+Sans+Condensed:800",
		"Fira+Sans+Condensed:800italic",
		"Fira+Sans+Condensed:900",
		"Fira+Sans+Condensed:900italic",
		...
		"Zeyada",
		"Zilla+Slab:300",
		"Zilla+Slab:300italic",
		"Zilla+Slab:400",
		"Zilla+Slab:400italic",
		"Zilla+Slab:500",
		"Zilla+Slab:500italic",
		"Zilla+Slab:600",
		"Zilla+Slab:600italic",
		"Zilla+Slab:700",
		"Zilla+Slab:700italic",
