...
if(!class_exists('Redux_Options') ){
    ...
    $fslashed_dir = trailingslashit(str_replace('\\','/', IRON_MUSIC_DIR_PATH . "admin/options/"));
    ...
    class Redux_Options {
        ...
        */
        function __construct($sections = array(), $args = array(), $extra_tabs = array()) {
            ...
            $defaults['menu_title'] = esc_html__('Options', 'sonaar');
            $defaults['page_icon'] = 'fa fa-themes';
            $defaults['page_title'] = esc_html__('Options', 'sonaar');
            ...
            $defaults['footer_credit'] = wp_kses( sprintf(__('<span id="footer-thankyou">Options panel created using <a href="%s" target="_blank">Redux Framework</a> v%s</span>',
             'sonaar'), esc_url($this->framework_url), $this->framework_version),iron_get_allowed_html());
        ...
        */
        function show($opt_name, $default = '') {
            ...
            if(!is_array($option) && $option != '') {
                ...
                echo wp_kses_post( $this->_get_std($opt_name, $default) );
            }
        }
            ...
            if($this->args['page_type'] == 'submenu') {
                ...
                if(true === $this->args['allow_sub_menu']) {
                    ...
                    if(true === $this->args['show_import_export']) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            esc_html__('Import / Export', 'sonaar'),
                            esc_html__('Import / Export', 'sonaar'),
                            $this->args['page_cap'],
                    ...
                    if(true === $this->args['dev_mode']) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            esc_html__('Dev Mode Info', 'sonaar'),
                            esc_html__('Dev Mode Info', 'sonaar'),
                            $this->args['page_cap'],
        ...
        */
        function _enqueue() {
			...
			/*/wp_register_style(
				'redux-font-awesome',
				$this->url . 'fontawesome/css/fontawesome-all.min.css',
				array(),
				time(),
				'all'
			);*/
            ...
            wp_localize_script('redux-opts-js', 'redux_opts', array('reset_confirm' => esc_html__('Are you sure? You will lose all custom values.', 'sonaar'), 'opt_name' => $this->args['opt_name']));
        ...
        */
        function _options_page_html() {
            ...
            echo ' &nbsp; ';
            submit_button(esc_html__('Reset to Defaults', 'sonaar'), 'secondary', $this->args['opt_name'] . '[defaults]', false);
            ...
            if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('redux-opts-saved') == '1') {
                if(isset($this->options['imported']) && $this->options['imported'] == 1) {
                    echo '<div id="redux-opts-imported"><strong>' . apply_filters('redux-opts-imported-text-' . $this->args['opt_name'], esc_html__('Settings Imported!', 'sonaar')) . '</strong></div>';
                } else {
                    echo '<div id="redux-opts-save"><strong>' . apply_filters('redux-opts-saved-text-' . $this->args['opt_name'], esc_html__('Settings Saved!', 'sonaar')).'</strong></div>';
                }
                delete_transient('redux-opts-saved');
            }

            echo '<div id="redux-opts-save-warn"><strong>' . apply_filters('redux-opts-changed-text-' . $this->args['opt_name'], esc_html__('Settings have changed, you should save them!', 'sonaar')) . '</strong></div>';
            echo '<div id="redux-opts-field-errors"><strong><span></span> ' . esc_html__('error(s) were found!', 'sonaar') . '</strong></div>';
            echo '<div id="redux-opts-field-warnings"><strong><span></span> ' . esc_html__('warning(s) were found!', 'sonaar').'</strong></div>';
            ...
            foreach($this->sections as $k => $section) {
				if($this->args['icon_type'] == 'image' || (isset($section['icon_type']) && $section['icon_type'] == 'image')) {
					...
					$icon = (!isset($section['icon'])) ? '<i class="fa fa-cog' . $icon_class . '"></i> ' : '<i class="fa-' . $section['icon'] . $icon_class . '"></i> ';
				} else {
					$icon_class = (!isset($section['icon_class'])) ? '' : ' ' . $section['icon_class'];
					$icon = (!isset($section['icon'])) ? '<i class="fa fa-cog' . $icon_class . '"></i> ' : '<i class="fa-' . $section['icon'] . $icon_class . '"></i> ';
				}
            ...
            if(true === $this->args['show_import_export']) {
				...
				if( isset($this->args['import_icon_type']) && $this->args['import_icon_type'] == 'image') {
					...
					$icon = (!isset($this->args['import_icon'])) ? '<i class="fas fa-sync-alt' . $icon_class . '"></i>' : '<i class="fa-' . $this->args['import_icon'] . $icon_class . '"></i> ';
				}
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="redux-opts-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span>' . esc_html__('Import / Export', 'sonaar') . '</span></a>';
                echo '</li>';
            ...
            if(is_array($this->extra_tabs)) {
				foreach($this->extra_tabs as $k => $tab) {
					if($this->args['icon_type'] == 'image' || (isset($tab['icon_type']) && $tab['icon_type'] == 'image')) {
    	                ...
    	                $icon = (!isset($tab['icon'])) ? '<i class="fa fa-cog' . $icon_class . '"></i> ' : '<i class="fa-' . $tab['icon'] . $icon_class . '"></i> ';
					} else {
						$icon_class = (!isset($tab['icon_class'])) ? '' : ' ' . $tab['icon_class'];
    	                $icon = (!isset($tab['icon'])) ? '<i class="fa fa-cog' . $icon_class . '"></i> ' : '<i class="fa-' . $tab['icon'] . $icon_class . '"></i> ';
					}
            ...
            if(true === $this->args['dev_mode']) {
				...
				if( isset($this->args['dev_mode_icon_type']) && $this->args['dev_mode_icon_type'] == 'image') {
					...
					$icon = (!isset($this->args['dev_mode_icon'])) ? '<i class="fa fa-info' . $icon_class . '"></i>' : '<i class="fa-' . $this->args['dev_mode_icon'] . $icon_class . '"></i> ';
				}
                echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="redux-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span>' . esc_html__('Dev Mode Info', 'sonaar') . '</span></a>';
                echo '</li>';
            }
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<h3>' . esc_html__('Import / Export Theme Options', 'sonaar') . '</h3>';
                echo '<h5>' . esc_html__('This will not import or export the content of your website but rather the settings of all the tabs above such as the look and feel, custom css, menu options, etc...', 'sonaar') . '</h4>';
                echo '<h4>' . esc_html__('Import Options', 'sonaar') . '</h4>';
                echo '<p><a href="javascript:void(0);" id="redux-opts-import-code-button" class="button-secondary">' . esc_html__('Import from file', 'sonaar') . '</a> <a href="javascript:void(0);" id="redux-opts-import-link-button" class="button-secondary">' . esc_html__('Import from URL', 'sonaar') . '</a></p>';

                echo '<div id="redux-opts-import-code-wrapper">';

                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-code-description">' . apply_filters('redux-opts-import-file-description', esc_html__('Input your backup file below and hit Import to restore your sites options from a backup.', 'sonaar')) . '</p>';
                echo '</div>';
                ...
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-link-description">' . apply_filters('redux-opts-import-link-description', esc_html__('Input the URL to another sites options set and hit Import to load the options from that site.', 'sonaar')) . '</p>';
                echo '</div>';
                ...
                echo '<p id="redux-opts-import-action"><input type="submit" id="redux-opts-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . esc_html__('Import', 'sonaar') . '"> <span>' . apply_filters('redux-opts-import-warning', esc_html__('WARNING! This will overwrite any existing options, please proceed with caution!', 'sonaar')) . '</span></p>';
                echo '<div id="import_divide"></div>';

                echo '<h4>' . esc_html__('Export Options', 'sonaar') . '</h4>';
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description">' . apply_filters('redux-opts-backup-description', esc_html__('Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'sonaar')).'</p>';
                echo '</div>';

                if ( !defined('AUTH_KEY') )
                    define('AUTH_KEY', 'Gh:XL6K4WVz 6Y&3z>^L3+X[&{<6KK-Y|MTZ|< wXXs}IUH.|F/pzWU2{;OBz{)V');

                if ( !defined('SECURE_AUTH_KEY') )
                    define('SECURE_AUTH_KEY',  '||KV>BF,#N&pyG+hWs#KmN%Vg|/X~O&Zu<086AfBh3N (vu_?h9BBIrFDu4$m5%C');

                echo '<p><a href="javascript:void(0);" id="redux-opts-export-code-copy" class="button-secondary">' . esc_html__('Copy', 'sonaar') . '</a> <a href="'.add_query_arg(array('feed' => 'reduxopts-'.$this->args['opt_name'], 'action' => 'download_options', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), esc_url(home_url('/'))).'" id="redux-opts-export-code-dl" class="button-primary">Download</a> <a href="javascript:void(0);" id="redux-opts-export-link" class="button-secondary">' . esc_html__('Copy Link', 'sonaar') . '</a></p>';
                ...
                echo '</textarea>';
                echo '<input type="text" class="large-text" id="redux-opts-export-link-value" value="' . add_query_arg(array('feed' => 'reduxopts-' . $this->args['opt_name'], 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), home_url('/')) . '" />';
            ...
            if(is_array($this->extra_tabs)) {
                foreach($this->extra_tabs as $k => $tab) {
                    echo '<div id="' . $k . '_section_group' . '" class="redux-opts-group-tab">';
                    echo '<h3>' . wp_kses_post( $tab['title'] ) . '</h3>';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<h3>' . esc_html__('Dev Mode Info', 'sonaar') . '</h3>';
                echo '<div class="redux-opts-section-desc">';
            ...
            if(isset($this->args['share_icons'])) {
                ...
                foreach($this->args['share_icons'] as $link) {
                    echo '<a href="' . esc_url($link['link']) . '" title="' . $link['title'] . '" target="_blank"><img src="' . $link['img'] . '"/></a>';
                }
                echo '</div>';
            }
