...
if ( ! class_exists( 'GambitRowSeparator' ) ) {
	...
	class GambitRowSeparator {
		...
		public function create_shortcode() {
			...
			vc_add_shortcode_param( 'shape_selector', 'sonaar_param_shape_selector', get_template_directory_uri() . '/includes/row-separators/row-separators/js/shape_selector.js' );
				function sonaar_param_shape_selector( $settings, $value ) {
					$all_shapes = gambit_row_separators_get_svg_names();
					$itemsSvg = gambit_row_separators_get_svgs();

					$output = '<div class="shape-selector" data-location="top">';

					foreach ($all_shapes as $shape){
		
						$output .= '<div  class="wpb_vc_param_value shape-selector-item" data-value="'. $shape .'" ' . (($shape == $value) ? 'selected' : '') . ' >';
						$output .= '<div class="svg-box">';
						$output .= '<svg viewBox="0 0 1600 ' . $itemsSvg[ $shape ]['height'] . '">'. $itemsSvg[ $shape ]['svg'] . '</svg></div>';
						$output .= '<div class="shape-selector-item-name">'. $shape .'</div></div>';
					...
					$output .= '<span class="shape-selector-color-1">' . __( 'Decoration 1 Color', GAMBIT_ROW_SEPARATORS ) . '</span>';
					$output .= '<span class="shape-selector-color-2">' . __( 'Decoration 2 Color', GAMBIT_ROW_SEPARATORS ) . '</span>';
					$output .= '<span class="shape-selector-color-3">' . __( 'Main Separator Fill Color', GAMBIT_ROW_SEPARATORS ) . '</span>';
			...
			vc_map( array(
			    ...
			    'base' => 'row_separator',
				'icon' => get_template_directory_uri() . '/includes/row-separators/row-separators/images/vc-row_separator.png',
				'description' => __( 'A cool top/bottom separator for your row', GAMBIT_ROW_SEPARATORS ),
				//'admin_enqueue_css' => get_template_directory_uri() . '/includes/row-separators/row-separators/row-separators/css/admin.css',
				'category' => __( 'Row Adjustments', GAMBIT_ROW_SEPARATORS ),
			    'params' => array(
					array(
						'type' => 'dropdown',
						'holder' => 'span',
	                    ...
	                    'description' => __( 'For best results, ensure that your WPBakery Page Builder row is set to Stretch', GAMBIT_ROW_SEPARATORS ),
					),
					array(
						'type' => 'shape_selector',
						'holder' => 'span',
...
&lt;path class='gambit_sep_main' d='M-8,100c0,0,197-74,406-74s604,170,800,170s412-96,412-96v110H-8V100z'/></pre>",
		...
		public function render_shortcode( $atts, $content = null ) {
			...
			wp_enqueue_style( __CLASS__, get_template_directory_uri() . '/includes/row-separators/row-separators/css/style.css', array(), VERSION_GAMBIT_ROW_SEPARATORS );
			wp_enqueue_script( __CLASS__, get_template_directory_uri() . '/includes/row-separators/row-separators/js/min/script-min.js', array(), VERSION_GAMBIT_ROW_SEPARATORS, true );
			...
			if ( ! empty( $atts['color1'] ) ) {
				$dataAttr .= 'data-bg-color-1="' . $atts['color1'] . '"';
			}
			if ( ! empty( $atts['color2'] ) ) {
				$dataAttr .= 'data-bg-color-2="' . $atts['color2'] . '"';
			}
			if ( ! empty( $atts['color3'] ) ) {
				$dataAttr .= 'data-bg-color-3="' . $atts['color3'] . '"';
			}
