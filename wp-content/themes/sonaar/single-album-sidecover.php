<?php
/**
* Template Name: Album with Side Cover
...
if ( ! function_exists( 'elementor_theme_do_location' ) || ! elementor_theme_do_location( 'single' ) ) {
	...
	<?php $nobanner = ( Iron_sonaar::getField('album_background_type', $post->ID ) == 'default' )? 'no-banner':''; ?>
	<?php $playerLocation = Iron_sonaar::getField( 'player_position', $post->ID ); ?>

			<?php if ( Iron_sonaar::getField('album_background_type', $post->ID ) !== 'default' ): ?>
		...
		( empty( $iron_sonaar_hide_page_title ) )? $title = 'title-album-enabled ': $title = '';
		?>
		<div id="post-<?php the_ID(); ?>" <?php post_class( 'featured ' . $title . $nobanner ); ?>>
	...
	<div class="sr_it-singlealbum-content-wrapper">
		...
		if ( Iron_sonaar::getField( 'alb_tracklist', $post->ID ) !== '0' && Iron_sonaar::getField( 'alb_tracklist', $post->ID ) !== '' ): //Since ACF 5.9.5 -> Iron_soundrise::getField( 'alb_tracklist', $post->ID ) == '' when playlist doesnt have track rather than '0'
			...
			the_widget('Iron_Widget_Radio', $iron_sonaar_atts, array( 'before_widget'=>'<article class="srt_player-container iron_widget_radio">', 'after_widget'=>'</article>', 'widget_id'=>'single_album_thumb'));
		endif;
		endif;
		?>
